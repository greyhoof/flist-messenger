...
FLoginWindow::FLoginWindow(QWidget *parent) : QWidget(parent), ui(new Ui::FLoginWindow) {
    ui->setupUi(this);
...
void FLoginWindow::setLoginCredentials(QString name, QString password) {
...
void FLoginWindow::loginClicked() {
    ...
    bool store = ui->autoLogin->isChecked();

    if (un.isEmpty() || pass.isEmpty()) {
        showError("Please enter both username and password");
    } else {
        emit loginRequested(un, pass, store);
    }
}
...
void FLoginWindow::enterPressed() {
    ...
    if (cw == ui->loginPage) {
        ...
        QMessageBox::critical(this, "This shouldn't happen",
                              "You somehow managed to press enter while a nonexistent page was selected. Please file a bug report explaining what you just did.");
    }
}
